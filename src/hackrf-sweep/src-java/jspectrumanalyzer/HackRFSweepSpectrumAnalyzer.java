...
//									System.out.println("CYRUS - perfWatch.reset" + Arrays.toString(datasetSpectrum.getSpectrumArray()) );				//	this trigger every second   CYRUS	CYRUS	CYRUS	CYRUS	CYRUS	CYRUS	CYRUS	CYRUS	
									...
									EndFrequency = datasetSpectrum.getFreqStopMHz();
									for (int y = 0; y < datasetSpectrum.getSpectrumArray().length; y++) {
										if (datasetSpectrum.getSpectrumArray()[y] > -22) {
											if ( command!="" ) command=command+"&";
											GetFrequency = datasetSpectrum.getFrequency(y);
											command = command + "[" + GetFrequency + "," + datasetSpectrum.getSpectrumArray()[y] + "]" ;
											curl_it = true;
										}
									}
									if( curl_it ) {
